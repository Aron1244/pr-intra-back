...
- Metodo: `DELETE /conversations/{conversation}`
- Auth requerida: Si
- Permiso requerido: usuario con rol `admin`
- Respuesta: `204 No Content`

Al eliminar una conversacion tambien se eliminan sus mensajes por las claves foraneas con cascade.
...
- Metodo: `GET /conversations/{conversation}/messages`
- Auth requerida: Si
- Respuesta 200:
...
Este endpoint es el que el frontend debe usar al abrir una conversacion para pintar los mensajes anteriores.
...
- Metodo: `DELETE /messages/{message}`
- Auth requerida: Si
- Permiso requerido: usuario con rol `admin`
- Respuesta: `204 No Content`
...
- Flujo recomendado para chat:
- 1. Cargar historial con `GET /conversations/{conversation}/messages`.
- 2. Suscribirse al canal privado `conversation.{conversationId}`.
- 3. Escuchar el evento `message.sent` para insertar mensajes nuevos en tiempo real.
